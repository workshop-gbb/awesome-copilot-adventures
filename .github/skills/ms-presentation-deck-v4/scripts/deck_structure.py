...
.deck-progress { background:var(--ps-deck-accent,var(--ps-color-ms-blue-500)); }
.lang-switcher__btn[aria-current="true"],
body[data-slide-theme="dark"] .lang-switcher__btn[aria-current="true"] { background:var(--ps-deck-accent,var(--ps-color-ms-blue-500)); color:#000000; }
...
.slide[data-ps-archetype="cover"].slide--dark .title { color:var(--ps-color-dark-ink); }
.slide[data-ps-archetype="cover"] .accent-theme, .slide[data-ps-archetype="closing"] .accent-theme { color:var(--accent); }
...
def install(markup: str, skeleton: str | None = None, *, theme_accent: str | None = None) -> str:
    theme_css = ""
    if theme_accent is not None:
        if theme_accent not in ACCENTS:
            raise ValueError("Choose a Microsoft theme accent: red, green, yellow or blue")
        markup = re.sub(r'<meta name="ps-deck-accent"[^>]*>\s*', "", markup)
        markup = markup.replace(
            "</head>", f'<meta name="ps-deck-accent" content="{theme_accent}">\n</head>', 1)
        theme_css = f":root {{ --ps-deck-accent: {ACCENTS[theme_accent]}; }}\n"
    ...
    return before + f'<style id="ps-structure-css">{CODE_BACKGROUND_CSS}{theme_css}{CSS}</style>\n<script id="ps-structure-runtime">{RUNTIME}</script>\n' + separator + after
...
def normalize(markup: str, registry: dict, *, cover, title, version: str, date: str,
              profile: str, closing=None, refresh_runtime: bool = False,
              theme_accent: str = "blue") -> str:
    ...
    return install(result, skeleton=builder.skel, theme_accent=theme_accent)
...
def issues(markup: str, registry: dict | None) -> list[str]:
    ...
    accent_meta = soup.select_one('meta[name="ps-deck-accent"]')
    accent = accent_meta.get("content") if accent_meta else None
    if accent not in ACCENTS:
        failures.append("Deck must declare one of the four Microsoft theme accents")
    ...
    cover = slides[0]
    if cover.get("data-ps-archetype") != "cover" or "slide--dark" not in cover.get("class", []):
        failures.append("Cover must use the dark shared cover for exact brand-color contrast")
    spans = cover.select("h1.title > span")
    if len(spans) != 2 or "accent-theme" not in spans[-1].get("class", []):
        failures.append(
            "Cover must have exactly two title lines, with the second in the theme accent")
    if accent in ACCENTS:
        for node in (cover, slides[-1]):
            declaration = re.search(r"(?:^|;)\s*--accent\s*:\s*([^;]+)", node.get("style", ""))
            if not declaration or declaration[1].strip() != ACCENTS[accent]:
                failures.append("Cover and closing must use the declared Microsoft accent token")
    ...
    if not closing.select_one("h1 > .accent-theme"):
        failures.append("Closing must retain the theme accent")

...
class DeckUXParser(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        if pairs.get('id'):
            self.ids.add(pairs['id'])
        if tag == 'use':
            reference = pairs.get('href') or pairs.get('xlink:href') or ''
            if not reference.startswith('#') or len(reference) == 1:
                self.error('SVG use must reference an embedded element', line)
            else:
                self.svg_references.append((reference[1:], line))
    ...
    def validate_references(self):
        for identifier, line in self.svg_references:
            if identifier not in self.ids:
                self.error(f'SVG use references a missing icon or element: #{identifier}', line)

...
def statement_photo(text_html, sub_html, img_src, source, alt, round_img=False, photo_left=False, *, asset_kind=None, fit='cover'):
    ...
    if fit not in {'cover', 'contain'}:
        raise ValueError('Statement image fit must be cover or contain')
    if round_img and fit == 'contain':
        raise ValueError('A circular crop cannot preserve the full image')
    cls = 'stmt__img' + (' stmt__img--round' if round_img else '') + (' stmt__img--contain' if fit == 'contain' else '')
    img = _media_image(img_src, cls, alt, asset_kind)
...
def toggle(labels, panels, *, initial=0, empty=''):
    if not labels or len(labels) != len(panels) or (initial is not None and not 0 <= initial < len(panels)):
        raise ValueError('Toggle requires matching labels and panels and a valid initial index')
    if initial is None and not str(empty).strip():
        raise ValueError('A neutral toggle requires an instruction')
    uid = f'toggle-{next(_UID)}'
    sw = ''.join(f'<button aria-pressed="{"true" if i == initial else "false"}" aria-controls="{uid}-panel-{i}">{l}</button>' for i, l in enumerate(labels))
    ps = ''.join(f'<div id="{uid}-panel-{i}" class="toggle__p{" on" if i == initial else ""}" aria-hidden="{"false" if i == initial else "true"}">{p}</div>' for i, p in enumerate(panels))
    neutral = f'<div class="ipanel toggle__empty" aria-live="polite"{"" if initial is None else " hidden"}>{empty}</div>' if empty else ''
    return f'<div class="toggle"><div><div class="toggle__sw" role="group">{sw}</div></div><div class="toggle__panels" aria-live="polite">{neutral}{ps}</div></div>'

...
class Deck:
    def __init__(self, version='1.0.0', title=('deck', 'deck', 'deck'), file_stem='Deck', skeleton=None, who_snippet=None, date=None, profile='standard', theme_accent='blue'):
        self.version = version
        self.stem = file_stem
        self.date = date or datetime.date.today().isoformat()
        self.profile = normalize_profile(profile)
        if theme_accent not in ACC:
    ...
    def cover(self, l1, l2, notes=None):
        ...
        l1 = t3(l1)
        l2 = t3(l2)
        self.add(f'<section class="slide slide--dark cover2" data-active="true" data-ps-family="structure" data-ps-archetype="cover" style="--accent: {ACC[self.theme_accent]}; --label: var(--ps-label-ms);"><h1 class="title"><span {self.reg(f"{k}.l1", *l1)}>{l1[0]}</span><span class="accent-theme" {self.reg(f"{k}.l2", *l2)}>{l2[0]}</span></h1></section>',
    ...
    def closing(self, t1, t2, tagline=TAGLINE, code_title='', code_lines=(), notes=None):
        ...
        self.add(f'''<section class="slide slide--dark" data-ps-family="structure" data-ps-archetype="closing" style="--accent: {ACC[self.theme_accent]}; --label: var(--ps-label-ms);">{deco(999, True)}
  <div class="eyebrow" {self.reg(f"{k}.eyebrow", "Closing", "Fechamento", "Cierre")}>Closing</div>
  <h1 class="section-title" data-fitlines style="margin-bottom: 20px;"><span {self.reg(f"{k}.t1", *t1)}>{t1[0]}</span><span class="accent-theme" {self.reg(f"{k}.t2", *t2)}>{t2[0]}</span></h1>
  ...
  <div style="display: grid; {grid} gap: 48px; align-items: start;">
    ...
    def _agenda_html(self):
        ...
        return (f'<section class="slide slide--light slide--snug" data-ps-family="structure" data-ps-archetype="agenda" style="--accent: {ACC[self.theme_accent]}; --label: var(--ps-label-ms);"><div class="eyebrow" {self.reg("agenda.eyebrow", "Agenda · click to jump", "Agenda · clique para pular", "Agenda · clic para saltar")}>Agenda · click to jump</div>'
    ...
    def html(self):
        ...
        return deck_structure.install(typo.apply(h), skeleton=self.skel, theme_accent=self.theme_accent)

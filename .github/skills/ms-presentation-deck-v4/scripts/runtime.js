...
(function () {
  ...
  function translateImageDescriptions() {
    document.querySelectorAll('img[data-alt-k]').forEach(image => {
      const value = tr(image, 'alt');
      if (typeof value !== 'string' || !value.trim()) {
        throw new Error('Missing image description: ' + image.dataset.altK);
      }
      image.alt = value;
    });
  }
  langRepaint.push(translateImageDescriptions);
  translateImageDescriptions();
  ...
  document.querySelectorAll('.assess').forEach(a => {
    ...
    function update() {
      ...
      const unanswered = rows.find(r => !r.dataset.v);
      const next = firstNo ? tr(firstNo, 'next') : unanswered ? unanswered.querySelector('span').innerHTML : null;
      panel.innerHTML = '<div class="ipanel__k">' + tr(panel, 'k') + ' · ' + answered.length + ' / ' + rows.length + '</div><div class="ipanel__v">' + (ladder[yes] ? tr(ladder[yes], 'name') : '') + '</div><div class="ipanel__t">' + (next ? tr(panel, 'next') + ' <b>' + next + '</b>' : tr(panel, 'top')) + '</div>';
    }
  ...
  document.querySelectorAll('.toggle').forEach(t => {
    ...
    const initial = btns.findIndex(b => b.getAttribute('aria-pressed') === 'true');
    const empty = t.querySelector('.toggle__empty');
    function select(i) {

...
C.statement_photo(text_html, sub_html, img_src, source, alt, round_img=False, photo_left=False, asset_kind=None, fit='cover')
...
C.toggle([label_before, label_after], [panel_html_before, panel_html_after], initial=None, empty=instruction_html)
...
- `statement_photo(..., fit='contain')` preserves the full image instead of cropping it into
  the default frame. It cannot be combined with a circular crop. The catalog's statement/image
  composition demonstrates this variant. Generated contextual imagery must be identified as
  generated, not as an archival screenshot or factual product evidence.
- Images can carry `data-alt-k="group.description"` pointing to a complete trilingual
  `Deck.reg` entry. The shared runtime updates the alt description when the language changes.

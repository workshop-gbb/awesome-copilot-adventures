...
## Theme-color correction (2026-09-07)

- `Deck(theme_accent=...)` selects one of the four exact Microsoft logo colors from the topic:
  red `#F25022`, green `#7FBA00`, yellow `#FFB900`, blue `#00A4EF`.
- Cover and closing share `.accent-theme`; they no longer force `.accent-blue` or replace the
  brand accent with a darker link/text color.
- Uniform black behind the structural titles preserves contrast in all four variants.
- `ps-deck-accent` records the choice and the structural gate checks it. Earlier blue-only/light
  cover rules below describe history, not the current contract.

...
- Legacy normalization installs the canonical dark token as well. The rendered QA gate checks
  both the dark slide and its outer canvas; retaining an old `#141414` token is a failure even if
  the structural classes and theme-accent metadata are correct.
...
| Slide | Rule |
|---|---|
| Cover | `.cover2`: exactly two complete sentence lines, second in the selected Microsoft logo color, Inter Medium 500 at 82px, uniform black canvas for contrast, no eyebrow, subtitle, date, logo block, or metadata. |
...
- Declare `Deck(theme_accent='red'|'green'|'yellow'|'blue')` from the topic's meaning.
  Cover and closing share that exact logo color through `.accent-theme` and `--accent`;
  blue is not the universal choice. The builder records `ps-deck-accent`.
- Never substitute a darker text/link tone for the brand accent. Use a contrasting neutral
  surface instead: structural titles sit on uniform black, so all four exact colors remain
  legible. Small functional links and labels may still use the separate `--ps-text-*` roles.
- Risk or limits can justify red; verified results green; decisions or iteration yellow;
  platform or integration blue. Record the reason, rather than cycling colors for variety.

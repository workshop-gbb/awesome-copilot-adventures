...
C.toggle(labels=[...], panels=[...], initial=None, empty=instruction_html)  # choose before revealing
...
- Exploratory toggles use `initial=None` and a translated instruction. They return to that neutral
  state on re-entry. The default first panel remains supported for legacy authored comparisons;
  it is not the choice for a no-spoiler exercise.
- Assessments never announce completion after a partial set of yes answers. Until all questions
  are answered, the first unanswered question remains the next action when no answer is no.
  A no answer takes priority with its corrective action. The result follows live locale changes.

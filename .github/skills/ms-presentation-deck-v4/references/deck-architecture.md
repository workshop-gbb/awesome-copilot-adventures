...
D = Deck(version='1.0.0', title=('en', 'pt', 'es'), file_stem='TopicCamelCase', profile='technical', theme_accent='blue')
...
`Deck(theme_accent=...)` records `ps-deck-accent` and the exact Microsoft color token in
`--ps-deck-accent`. Choose red, green, yellow or blue from the topic's meaning, not a fixed
blue template. The progress line, selected locale, cover and closing share that choice.
Selected-locale text uses black on the exact accent fill for small-text contrast. The two structural
title lines sit on uniform black, with `.accent-theme` using the exact logo color rather
than a darker `--ps-text-*` link tone. Content parts retain their semantic `--accent`.
...
- Presenter controls, current/next labels and help are localized along with the notes.
  The presenter copies the main document's embedded font faces locally and receives the theme
  accent over the same channel; it must not fetch fonts or lose translation offline.
...
Review complete EN/PT-BR/ES contact sheets; geometry,
typography and SVG gates also cover all three locales. Incomplete topics remain explicitly

...
1. **Definir o perfil, a audiência e o acento temático.** Escolher `Deck(profile=...,
   theme_accent=...)`, duração, contexto de uso, decisão esperada e nível técnico.
   O acento é uma das quatro cores exatas do logo Microsoft, escolhido pelo assunto e significado,
   não por rodízio nem por um padrão universal azul.
...
D = Deck(
    ...
    profile='technical',
    theme_accent='blue',  # plataforma; escolher red, green, yellow ou blue pelo tema
)
D.cover(line1, line2)
D.agenda()
...
- Capa: duas frases completas, a segunda na cor temática exata do logo Microsoft, nada além do
  título. Capa e fechamento usam o mesmo `theme_accent`: `red`, `green`, `yellow` ou `blue`.
  Não fixar azul para todos os temas nem trocar a cor da marca por um tom de link ou texto.
  A capa usa o preto uniforme para preservar contraste nas quatro variantes sem escurecer a cor.
...
- Acento temático: vermelho `red` = `#F25022`; verde `green` = `#7FBA00`; amarelo
  `yellow` = `#FFB900`; azul `blue` = `#00A4EF`. O azul da marca não é azul-marinho,
  azul Fluent nem um tom derivado de `--ps-text-blue`.
- Escolher e registrar a cor pelo tema: risco/limite pode pedir vermelho; correção/resultado,
  verde; decisão/iteração, amarelo; plataforma/integração, azul. A semântica do conteúdo prevalece.
  Partes, séries e estados podem usar as outras cores quando tiverem significado próprio.
- Distinguir acento de marca de texto funcional: tons `--ps-text-*` são reservados a links e
  rótulos pequenos que exigem contraste, não à linha de destaque da capa ou do fechamento.
  Se a cor exata não tiver contraste no fundo escolhido, mudar a superfície ou usar texto neutro
  com um acento separado; não alterar silenciosamente o RGB da marca.
...
11. O builder registra `ps-deck-accent`; o gate estrutural verifica a escolha entre as quatro
    cores e sua aplicação coerente na capa e no fechamento. Novos builders declaram
    `theme_accent=` explicitamente. A compatibilidade com builders antigos não autoriza
    escolher azul automaticamente para novos temas.

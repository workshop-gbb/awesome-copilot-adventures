...
| Capability evidence | No adventure-specific artifact or verifier output. | An artifact exists but does not prove the named capability. | The bounded stabilization loop passes `node labs/tempora-loop/verify.js`. | The evidence also demonstrates limitations, failure recovery, and independent reproducibility. |
...
- The bounded stabilization loop passes `node labs/tempora-loop/verify.js`.

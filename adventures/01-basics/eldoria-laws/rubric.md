...
| Capability evidence | No adventure-specific artifact or verifier output. | An artifact exists but does not prove the named capability. | Scoped repository instructions pass `node labs/eldoria-laws/verify.js` and are discovered from the starter workspace root. | The evidence also demonstrates limitations, failure recovery, and independent reproducibility. |
...
- Scoped repository instructions pass `node labs/eldoria-laws/verify.js` and are discovered from the starter workspace root.

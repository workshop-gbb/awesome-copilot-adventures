...
| Capability evidence | No adventure-specific artifact or verifier output. | An artifact exists but does not prove the named capability. | The reusable evidence skill passes `node labs/algora-skills/verify.js` and is discovered from the starter workspace root. | The evidence also demonstrates limitations, failure recovery, and independent reproducibility. |
...
- The reusable evidence skill passes `node labs/algora-skills/verify.js` and is discovered from the starter workspace root.

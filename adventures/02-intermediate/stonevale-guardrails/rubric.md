...
# Rubric: The Guardrails of Stonevale
...
| Capability evidence | No adventure-specific artifact or verifier output. | An artifact exists but does not prove the named capability. | The deterministic command policy passes `node labs/stonevale-guardrails/verify.js` against safe and unsafe cases. | The evidence also demonstrates limitations, failure recovery, and independent reproducibility. |
...
- The deterministic command policy passes `node labs/stonevale-guardrails/verify.js` against safe and unsafe cases.

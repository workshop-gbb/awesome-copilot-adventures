...
# Rubric: The Parallel Trials of Mythos
...
| Capability evidence | No adventure-specific artifact or verifier output. | An artifact exists but does not prove the named capability. | Parallel start, input validation, and deterministic result ordering pass `node labs/mythos-parallel/verify.js`. | The evidence also demonstrates limitations, failure recovery, and independent reproducibility. |
...
- Parallel start, input validation, and deterministic result ordering pass `node labs/mythos-parallel/verify.js`.

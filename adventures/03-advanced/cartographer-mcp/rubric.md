...
| Capability evidence | No adventure-specific artifact or verifier output. | An artifact exists but does not prove the named capability. | The local server completes initialize, tools/list, and tools/call through `node labs/cartographer-mcp/verify.js`. | The evidence also demonstrates limitations, failure recovery, and independent reproducibility. |
...
- The local server completes initialize, tools/list, and tools/call through `node labs/cartographer-mcp/verify.js`.

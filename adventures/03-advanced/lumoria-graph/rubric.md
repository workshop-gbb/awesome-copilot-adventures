...
# Rubric: The Knowledge Graph of Lumoria
...
| Capability evidence | No adventure-specific artifact or verifier output. | An artifact exists but does not prove the named capability. | Cycle-safe dependency and impact traversal passes `node labs/lumoria-graph/verify.js`. | The evidence also demonstrates limitations, failure recovery, and independent reproducibility. |
...
- Cycle-safe dependency and impact traversal passes `node labs/lumoria-graph/verify.js`.

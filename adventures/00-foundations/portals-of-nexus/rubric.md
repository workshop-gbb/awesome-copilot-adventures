...
| Capability evidence | No adventure-specific artifact or verifier output. | An artifact exists but does not prove the named capability. | A role, harness, target, and environment map passes `node labs/portals-of-nexus/verify.js`. | The evidence also demonstrates limitations, failure recovery, and independent reproducibility. |
...
- A role, harness, target, and environment map passes `node labs/portals-of-nexus/verify.js`.

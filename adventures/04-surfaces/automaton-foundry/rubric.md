...
| Capability evidence | No adventure-specific artifact or verifier output. | An artifact exists but does not prove the named capability. | The pinned Copilot SDK application and evaluation contract pass `node labs/automaton-foundry/verify.js`; runtime evidence is recorded when authentication is available. | The evidence also demonstrates limitations, failure recovery, and independent reproducibility. |
...
- The pinned Copilot SDK application and evaluation contract pass `node labs/automaton-foundry/verify.js`; runtime evidence is recorded when authentication is available.
